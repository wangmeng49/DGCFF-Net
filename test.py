...
def get_args():
    ...
    parser.add_argument("-c", "--config", type=str, default="configs\A2Net.py")
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for input in tqdm(test_loader):
            ...
            '''
            #模型的定义forword返回多个值的时候用这个,A2Net,DMINet,SEIFNet
            outputs = model(input[0].cuda(), input[1].cuda())
            raw_predictions = outputs[0]  # ✅ 假设你需要第一个输出
            mask = input[2].cuda()
            img_id = input[3]
            '''
            if cfg.net == 'SARASNet':
...
def main():
    ...
    # 检查读取权限
    if not os.access(args.ckpt, os.R_OK):
        raise PermissionError(f"No read access to the file: {args.ckpt}")

    # 测试文件读取
    print(f"Attempting to load checkpoint: {args.ckpt}")
    try:
