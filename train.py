...
class myTrain(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        preds = self(imgA, imgB)  # preds是tuple/list
        ...
        # 取第一个输出做预测指标计算
        if self.cfg.argmax:
            pred = preds[0].argmax(dim=1) if isinstance(preds, (tuple, list)) else preds.argmax(dim=1)
        else:
            pred_tensor = preds[0] if isinstance(preds, (tuple, list)) else preds
            pred = pred_tensor.squeeze(1) > 0.5
    ...
    def training_step(self, batch, batch_idx):
        imgA, imgB, mask = batch[0], batch[1], batch[2]
        #imgA, imgB, mask = batch[:3] #针对STADE网络
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.cfg.net == 'SARASNet':
            mask = Variable(resize_label(mask.data.cpu().numpy(),
                                         size=preds[0].data.cpu().numpy().shape[2:]).to('cuda')).long()
            param = 1
            # 对第二个输出调整值，保持逻辑
            if isinstance(preds, (tuple, list)):
                preds = tuple(p + param if i == 1 else p for i, p in enumerate(preds))
            else:
                preds[:, 1, :, :] = preds[:, 1, :, :] + param
        ...
        if self.cfg.argmax:
            ...
            if self.cfg.net == 'maskcd':
                ...
                pred_tensor = preds[0] if isinstance(preds, (tuple, list)) else preds
                loss = self.loss(pred_tensor, mask)
                pred = pred_tensor > 0.5
    ...
    def test(self, idx, value=None):
        for input in tqdm(self.test_loader):
            raw_predictions, mask_test = self(input[0].cuda(self.cfg.gpus[0]), input[1].cuda(self.cfg.gpus[0])), input[
                2].cuda(self.cfg.gpus[0])

            if self.cfg.net == 'SARASNet':
                mask_test = Variable(resize_label(mask_test.data.cpu().numpy(),
                                                  size=raw_predictions[0].data.cpu().numpy().shape[2:]).to(
                    'cuda')).long()
                param = 1
                if isinstance(raw_predictions, (tuple, list)):
                    raw_predictions = tuple(p + param if i == 1 else p for i, p in enumerate(raw_predictions))
                else:
                    raw_predictions[:, 1, :, :] = raw_predictions[:, 1, :, :] + param

            if self.cfg.argmax:
                pred_test = raw_predictions[0].argmax(dim=1) if isinstance(raw_predictions,
                                                                           (tuple, list)) else raw_predictions.argmax(
                    dim=1)
            else:
                if self.cfg.net == 'maskcd':
                    raw_prediction = raw_predictions[0] if isinstance(raw_predictions,
                                                                      (tuple, list)) else raw_predictions
                    pred_test = raw_prediction > value
                    pred_test = pred_test.squeeze(1)
                else:
                    pred_test_tensor = raw_predictions[0] if isinstance(raw_predictions,
                                                                        (tuple, list)) else raw_predictions
                    pred_test = pred_test_tensor > 0.5
    ...
    def test(self, idx, value = None):
        for input in tqdm(self.test_loader):
            #imgA, imgB, mask_test = input#针对STADE网络
            #raw_predictions = self(imgA.cuda(cfg.gpus[0]), imgB.cuda(cfg.gpus[0]))#针对STADE网络
